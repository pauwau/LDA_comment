...
def split_data(dialogue):
	dialogue.pop()
	# for utterance in dialogue:
	# 	x = utterance[0].encode("utf_8")
	# 	print x
	return (dialogue)
...
def ranking_data(dia_his,co_ID,utterance_list,dictionary,lda,utt_vec_lda_list):
	...
	for utterance,utt_vec_lda in zip(utterance_list,utt_vec_lda_list):
		if(isinstance(his_vec_lda,list) and isinstance(utt_vec_lda,list)):
			utterance_ranking.append([utterance[0],utterance[1],cosSimilarity(his_vec_lda,utt_vec_lda)])
...
def utt_vec_lda_listMake(utterance_list,dictionary):
	print ("lda_list making...!")
	utt_vec_lda_list = []
	for utterance in utterance_list:
		if(isinstance(utterance,tuple)):
			utt_vec_bow = dictionary.doc2bow(extractKeyword(atmarkDelete(utterance[0])))
			#print utt_vec_bow
		else:
			print("############################")
...
if __name__ == '__main__':
	...
	utterance_list = cur.execute( "select sentence,dialogue_id from c_short WHERE repfrom = 0").fetchall()
	rank_array = []
	rank_text = []
	utt_vec_lda_list = utt_vec_lda_listMake(utterance_list,dictionary)
	...
	print("ranking_under 1000:" + str(under1000))
